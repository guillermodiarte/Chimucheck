...
export function PodiumModal({ isOpen, onClose, winners, prizePool }: PodiumModalProps) {
  ...
  return (
    <>
      {/* Confetti placed outside the modal hierarchy to prevent canvas intrinsic size from blowing up the modal bounds! */}
      {isOpen && (
        <div className="fixed inset-0 pointer-events-none z-[150]">
          <Confetti key={confettiKey} width={windowSize.width} height={windowSize.height} numberOfPieces={400} recycle={false} />
        </div>
      )}
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-[95vw] md:max-w-3xl max-h-[95vh] h-auto pt-6 pb-10 bg-black/95 border-none flex flex-col items-center justify-center overflow-hidden z-[110]">
                  ...
                  <div className={`w-full rounded-t-lg flex flex-col items-center justify-start pt-4 text-black font-bold uppercase tracking-widest ${isFirst ? "h-64 md:h-72 bg-gradient-to-b from-yellow-400 to-yellow-600" :
                    ...
                    {(() => {
                      ...
                      return winner.players && winner.players.length > 0 && (
                        <div className="w-full flex-1 min-h-0 overflow-hidden relative mt-2 mb-2 flex flex-col items-center justify-start [mask-image:linear-gradient(to_bottom,transparent,black_10%,black_90%,transparent)]">
                          <style>{`
